...
bool IsBlockPayeeValid(const CBlock& block, int nBlockHeight)
{
    ...
    const CTransaction& txNew = (nBlockHeight > Params().LAST_POW_BLOCK() && block.vtx.size() >= 2 ? block.vtx[1] : block.vtx[0]);
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake)
{
    ...
    CAmount advisorsFee = GetAdvisorsFee(blockValue);
    ...
    if (hasPayment) {
	    ...
	    // advisors value + fee
            txNew.vout[i + 1].nValue = advisorsFee;
            txNew.vout[i + 1].scriptPubKey = GetAdvisorsFeeScript();

            //subtract mn payment and advisors fee from the stake reward
            if (i == 3) {
                CAmount p1 = ((masternodePayment) / 2 / CENT) * CENT;
                CAmount p2 = masternodePayment - p1;

                txNew.vout[i - 1].nValue -= p1;
                txNew.vout[i - 2].nValue -= p2;
